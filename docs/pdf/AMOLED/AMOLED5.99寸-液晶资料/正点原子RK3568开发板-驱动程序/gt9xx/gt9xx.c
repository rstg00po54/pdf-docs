...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    printk("test:2 finger=%d\r\n",finger); 
        printk("test:2 x-max=%d\r\n",ts->abs_x_max); 
            printk("test:2 y-max=%d\r\n",ts->abs_y_max); 
...
//    GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
    ...
    u8 test_buf[5] = {0x80, 0x41, 0x00, 0x56, 0xAA};
    gtp_i2c_write(ts->client, test_buf, 5);
    msleep(100); 
    u8 test_buf1[3] = {0x80, 0x40, 0xAA};
    gtp_i2c_write(ts->client, test_buf1, 3);
    msleep(100); 
    u8 test_buf2[241] = { 0x80, 0x50, 0x02, 0x38, 0x04, 0x70, 0x08, 0x3A, 0x3D, 0x04, 0x88, 0xE2,
...
0x10, 0x00, 0x23, 0x9D, 0x09, 0x26, 0x79, 0x23, 0x01};
    gtp_i2c_write(ts->client, test_buf2, 241);
    ...
    */
    ...
    if (ret < 0)
    {
        ...
        ts->abs_x_max = 1080;//gtp_config->gtp_resolution_x;
        ts->abs_y_max = 2160;//gtp_config->gtp_resolution_y;
